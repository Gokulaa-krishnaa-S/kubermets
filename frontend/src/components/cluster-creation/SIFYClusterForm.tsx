...
import template from '../../data/SIFYClusterForm.template.json';
...
export default function SIFYClusterForm({ clusterId, onSubmit, onCancel }: Props) {
  ...
  const handleSubmit = async (formData: any) => {
    ...
    try {
      ...
      console.error('Error submitting SIFY cluster form:', error);
      throw error;
    } finally {
      setLoading(false);
    }
    ...
    <div className="max-w-4xl mx-auto">
      <div className="mb-8">
        <h1 className="text-xl font-semibold">
          {clusterId ? 'Edit Sify Cluster' : 'Create Sify Cluster'}
        </h1>
        <p className="text-muted-foreground mt-1">
          Configure your Sify Kubernetes cluster settings
        </p>
      </div>
      <FormRenderer 
