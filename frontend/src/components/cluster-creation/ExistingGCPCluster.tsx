...
import { clusterCreationApi, fetchClusterDetails } from '../../services/clusterCreationApi';
...
export default function ExistingGCPCluster({ onSubmit, onCancel, editClusterId, initialData }: ExistingGCPClusterProps) {
  const navigate = useNavigate();
  const [formData, setFormData] = useState<any>({});
  const [loading, setLoading] = useState(false);
  const [computedTemplate, setComputedTemplate] = useState<any>(existingTemplate);
  ...
  useEffect(() => {
    ...
    const locationGroup = clone.fields.find((f: any) => f.name === 'location' && f.type === 'group');
    if (locationGroup && Array.isArray(locationGroup.fields)) {
      const regionField = locationGroup.fields.find((f: any) => f.name === 'region');
      if (regionField) {
        regionField.options = gcpRegions;
      }
      const zoneField = locationGroup.fields.find((f: any) => f.name === 'zone');
      if (zoneField) {
  ...
  // Function to extract project_id from service account JSON
  const extractProjectIdFromServiceAccount = (serviceAccountJson: any): string | null => {
    try {
      if (!serviceAccountJson || typeof serviceAccountJson !== 'object') {
        return null;
      }
      
      // Check for project_id field in the JSON (case-insensitive)
      const projectId = serviceAccountJson.project_id || serviceAccountJson.projectId || serviceAccountJson.PROJECT_ID;
      ...
      console.error('Error extracting project_id from service account JSON:', error);
      return null;
    }
  };
  ...
  const handleFormChange = (data: any) => {
    ...
    // Check if service account JSON was uploaded and extract project_id
    if (data.credentials?.service_account_json?.content) {
      const projectId = extractProjectIdFromServiceAccount(data.credentials.service_account_json.content);
      
      if (projectId) {
        // Auto-populate the googleProjectId field only if it's not already set
        if (!data.general?.googleProjectId) {
          const updatedData = {
            ...data,
            general: {
              ...data.general,
        ...
        // Show validation popup if project_id cannot be extracted
        setValidationModal({
          isOpen: true,
          message: 'Cannot fetch project ID from uploaded service account JSON. Please enter it manually.'
        });
      }
    }
  };
  ...
  const handleSubmit = async (data: any) => {
    try {
      ...
      let finalRegion = (locationGroup.region as string | undefined) || undefined;
      ...
      // Extract service account JSON content (remove the nested structure)
      const serviceAccountJson = data.credentials?.service_account_json?.content || data.credentials?.service_account_json;
      ...
      let response: any;
      if (editClusterId) {
        response = await clusterCreationApi.updateExistingCluster(editClusterId, payload);
      } else {
      ...
      setProgressModal({
        isOpen: true,
        type: 'success',
        message: editClusterId ? 'Existing cluster updated successfully!' : 'Existing cluster imported successfully!'
      });
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to import cluster. Please try again.';
          ...
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="text-lg font-semibold">
                {progressModal.type === 'progress' ? 'Processing...' : 
                 progressModal.type === 'success' ? 'Success!' : 'Error!'}
              </h3>
            </div>
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-semibold">
              {editClusterId ? 'Edit Existing GCP Cluster' : 'Import Existing GCP Cluster'}
            </h1>
