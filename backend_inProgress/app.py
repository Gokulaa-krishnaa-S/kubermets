...
def create_app():
    ...
    try:
        ...
        logger.error("Please ensure PostgreSQL is running and DATABASE_URL is correctly configured")
    
    # Setup background tasks if enabled
    enable_background = os.getenv('ENABLE_BACKGROUND_REFRESH', 'false').lower() == 'true'
...
def setup_background_tasks(app):
    ...
    try:
        ...
        scheduler.start()
        logger.info(f"Background scheduler started with {refresh_interval} minute refresh interval")
...
def health_check():
    ...
    try:
        ...
        if kubecost_domain:
            ...
            kubecost_status = "healthy" if response.status_code == 200 else f"unhealthy: {response.status_code}"
        else:
    ...
    health_data = {
        "status": "healthy" if db_status == "healthy" and "healthy" in kubecost_status else "unhealthy",
        "timestamp": datetime.utcnow().isoformat(),
        "services": {
            "database": db_status,
            "kubecost_api": kubecost_status
        },
        "background_tasks": {
            "enabled": os.getenv('ENABLE_BACKGROUND_REFRESH', 'false').lower() == 'true',
