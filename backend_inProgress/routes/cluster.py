...
def dashboard_summary():
    ...
    try:
        ...
        cluster_data = kubecost_service.get_cluster_data(window=window, force_refresh=force_refresh)
        node_data = kubecost_service.get_node_data(window=window, force_refresh=force_refresh)
        pod_data = kubecost_service.get_pod_data(window=window, force_refresh=force_refresh)
        ...
        if node_data.get("status") == "success" and node_data.get("data"):
            if isinstance(node_data["data"], dict) and "data" in node_data["data"]:
                ...
                summary["nodes"]["total_cost"] = sum(node.get("totalCost", 0) for node in node_list)
                # Calculate average efficiency
                efficiencies = [node.get("cpuEfficiency", 0) for node in node_list if node.get("cpuEfficiency")]
                summary["nodes"]["avg_efficiency"] = sum(efficiencies) / len(efficiencies) if efficiencies else 0
        ...
        if pod_data.get("status") == "success" and pod_data.get("data"):
            if isinstance(pod_data["data"], dict) and "data" in pod_data["data"]:
                ...
                summary["pods"]["total_cost"] = sum(pod.get("totalCost", 0) for pod in pod_list)
