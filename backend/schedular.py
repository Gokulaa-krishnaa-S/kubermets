...
def calculate_percentage(usage: Optional[float], request: Optional[float]) -> Optional[float]:
...
def extract_namespace_and_name(allocation_key: str) -> tuple[Optional[str], Optional[str]]:
...
def format_cluster_metrics(allocation_data: Dict, cluster_name: str = None) -> Dict:
    ...
    formatted_data = {
        ...
        "memory_gb_requested": bytes_to_gb(allocation_data.get('ramByteRequestAverage', 0)),
...
def format_node_metrics(allocation_data: Dict, node_name: str, cluster_name: str = None) -> Dict:
...
def format_pod_metrics(allocation_data: Dict, pod_key: str, cluster_name: str = None) -> Dict:
    ...
    cpu_efficiency = calculate_percentage(
        allocation_data.get('cpuCoreUsageAverage', 0),
        allocation_data.get('cpuCoreRequestAverage', 0)
    ) / 100 if allocation_data.get('cpuCoreRequestAverage', 0) > 0 else 0.0
    
    ram_efficiency = calculate_percentage(
        allocation_data.get('ramByteUsageAverage', 0),
        allocation_data.get('ramByteRequestAverage', 0)
    ) / 100 if allocation_data.get('ramByteRequestAverage', 0) > 0 else 0.0
...
def format_kubecost_response(kubecost_data: Dict, user_id: int, cluster_id: int) -> Dict:
    ...
    for snapshot_set in kubecost_data.get("data", {}).get("sets", []):
        ...
        for allocation_name, allocation_data in snapshot_set.get("allocations", {}).items():
            ...
            if allocation_name != "__idle__":
                ...
                if node_data and isinstance(node_data, dict):
                    ...
                    for node_set in node_data.get("data", {}).get("sets", []):
                        ...
                        for node_name, node_allocation in node_set.get("allocations", {}).items():
                            formatted_node_allocations[node_name] = {
                                **format_node_metrics(node_allocation, node_name, allocation_name),
                                "pod_data": None
                            }
                            ...
                            if pod_data and isinstance(pod_data, dict):
                                ...
                                for pod_set in pod_data.get("data", {}).get("sets", []):
                                    ...
                                    for pod_key, pod_allocation in pod_set.get("allocations", {}).items():
                                        formatted_pod_allocations[pod_key] = format_pod_metrics(
                                ...
                                formatted_node_allocations[node_name]["pod_data"] = {
                                    ...
                                    "data": {
                                        ...
                                        "window": pod_data.get("data", {}).get("window", {})
                                    }
                                }
                ...
                formatted_allocations[allocation_name] = format_cluster_metrics(allocation_data, allocation_name)
            ...
            for node_name in i["allocations"]:
                if node_name not in exclude:
                    try:
                        ...
                        log.error("Failed to fetch pod data for node %s: %s", node_name, e)
            ...
            for cluster_name in i.get("allocations", {}):
                if cluster_name != "__idle__":
                    try:
                        ...
                        log.error("Failed to fetch node data for cluster %s: %s", cluster_name, e)
...
def send_snapshots_to_backend(user_id: int, cluster_id: int, kubecost_data: Dict) -> None:
    """
...
def collect_once(cluster_cfg: Dict) -> None:
    ...
    for attempt in range(1, RETRY_ATTEMPTS + 1):
        try:
            ...
            log.warning("Collect failed (attempt %d/%d) | cluster=%s | err=%s",
                        attempt, RETRY_ATTEMPTS, cluster_name, e)
...
def initial_collect_all(scheduler: BackgroundScheduler):
    ...
    for cfg in clusters:
        try:
            ...
            log.error("Initial collect failed | cluster_id=%s err=%s", cfg.get("cluster_id"), e)
...
def main():
    ...
    log.info("Scheduler started. Backend=%s interval=%d min window=%d min",
             BACKEND_API_URL, COLLECTION_INTERVAL_MIN, COLLECTION_WINDOW_MIN)
